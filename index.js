...
function distributePotsForFolding(game) {
    ...
    let totalPot = game.pot;

    const remainingPlayerIndex = updatedPlayers.findIndex(player => player.inHand);

    if (remainingPlayerIndex !== -1) {
        updatedPlayers[remainingPlayerIndex].chips += totalPot;
        updatedPlayers[remainingPlayerIndex].amountWon = (updatedPlayers[remainingPlayerIndex].amountWon || 0) + totalPot;
    }

    updatedPlayers.forEach(player => {
        player.bet = 0;  // Reset bet for all players
    });
...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (playersInHand.length === 1) {
            ...
            distributePotsForFolding(game); // Ensure this function saves the updated game state
        } else {
            const allInConditionMet = checkAllInCondition(game);
            if (!allInConditionMet) {
                if (allPlayersHaveActed(game)) {
                    advanceGameStage(game); // Ensure this function saves the updated game state
                } else {
                    advanceTurn(game); // Ensure this function saves the updated game state
                }
            }
        }
